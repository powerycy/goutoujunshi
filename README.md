...
<details>
<summary>版本更新维护记录</summary>

| 日期 | 类型 | 更新 | 用户价值 |
| --- | --- | --- | --- |
| 2026-07-22 | 改进 | 聊天截图、导出的文字记录、复制粘贴的微信/QQ/社媒对话、带说话人标记的转录文本都会优先按聊天记录处理。 | 分析时先看原文、顺序、间隔和回应情况，减少把文字聊天误判成线下见面或把用户猜测写成对方原话。 |
| 2026-07-22 | 改进 | “我有没有接住情绪”“这段话怎么回”“他/她这句话什么意思”等问题默认进入聊天记录分析流程。 | 先评价情绪承接、关系信号和事实/推测边界，再给可发送回复与后续观察点。 |
| 2026-07-21 | 新增 | 投入失衡与体面退场模块。 | 处理“是不是只有我在投入”“要不要继续”“怎么降低投入或退出”，用互惠、兑现、主动修复、现实可行性和风险成本判断关系，而不是只看回复速度。 |
| 2026-07-21 | 新增 | 事件型观察窗口。 | 把观察条件绑定到下一次约见、对方忙完后的主动联系、是否给替代方案等具体事件，减少每天反复解读一个表情或一句短回复。 |
| 2026-07-21 | 新增 | 降级投入、结束关系和事务联系话术。 | 帮用户体面回收时间精力，并明确降级投入不是欲擒故纵、冷暴力或绕过拒绝。 |
| 2026-07-21 | 改进 | 覆盖同性/未出柜、异地、同事、双方同意的非单偶关系、分手后联系，以及自伤要挟、定位、跟踪、暴力等高风险场景。 | 复杂关系不被简单套模板；危险情境优先进入安全流程。 |
