...
| 日期 | 类型 | 更新 | 用户价值 |
| --- | --- | --- | --- |
| 2026-07-23 | 架构 | 收缩 `SKILL.md` 为轻量行为与路由内核；新增经典社交体系机制／风险与伦理能力转译，覆盖 Blueprint 内在状态、自然流、冷读和 Mystery 结构化互动；验证器改为必需文件、最低数量、上下文预算和运行时边界，增加白名单安装模式并补充回归场景。 | 只按当前问题加载 1–3 份资料，在保留实战颗粒度的同时降低日常上下文成本；安装副本不携带项目资料也能自检，并阻止读心、操控和阶段升级误用。 |
