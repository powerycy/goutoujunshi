...
| 知识文档数量 | 至少20份核心文档和20份实用文档 | 数量不足时失败 | `scripts/validate_skill.py` | 已有，CI要求 |
...
| 经典社交体系场景规范 | 覆盖冷读、自然流、结构化互动、按需加载及操控边界 | 缺少规范文件时失败 | `tests/classic-social-framework-scenarios.md` | 已有，CI要求 |
| Skill上下文预算 | 行数、字符数和保守Token估算不得超过预算 | 超出预算时失败 | `scripts/validate_skill.py` | 已有，CI要求 |
