...
function loadConfig(overrides = {}) {
  ...
  const config = {
    ...
    webDemoAutoLogin: bool('WEB_DEMO_AUTO_LOGIN', false),
    webDemoCodeHashes,
    webDemoAnalysisQuota: int('WEB_DEMO_ANALYSIS_QUOTA', 100),
    webDemoSessionSeconds: int('WEB_DEMO_SESSION_SECONDS', 2592000),
    webDemoDailyAnalysisAttempts: int('WEB_DEMO_DAILY_ANALYSIS_ATTEMPTS', 100),
    ...
    modelTimeoutMs: int('MODEL_TIMEOUT_MS', 120000),
  ...
  if (config.webDemoEnabled && !config.webDemoAutoLogin && !config.webDemoCodeHashes.length) throw new Error('WEB_DEMO_CODE_HASHES is required when WEB_DEMO_ENABLED=true and WEB_DEMO_AUTO_LOGIN=false')
  ...
  if (config.webDemoDailyAnalysisAttempts < 1) throw new Error('WEB_DEMO_DAILY_ANALYSIS_ATTEMPTS must be at least 1')
