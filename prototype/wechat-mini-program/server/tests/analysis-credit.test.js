...
test('只有 delivered 才核销，创建任务先原子预留防并发超发',async()=>{
  const context=createContext({databaseUrl:':memory:',config:{nodeEnv:'test',allowDevAuth:false,sessionSecret:'test-session-secret-long-enough',dataEncryptionKey:'test-data-key-long-enough',modelMode:'mock',betaCampaignQuota:10000}});const current=user(context)
  ...
  assert.throws(()=>context.analysis.create(current,payload('这是第三份足够长的测试案卷，因为前两份正在处理，应当不能继续超额预留。')),/没有可用/)
  await context.analysis.process(one.analysisId);await context.analysis.process(two.analysisId)
  ...
  assert.deepEqual({...row},{trial_analysis_total:2,trial_analysis_used:2,trial_analysis_reserved:0})
})

test('安全拦截不核销免费分析',async()=>{
  const context=createContext({databaseUrl:':memory:',config:{nodeEnv:'test',allowDevAuth:false,sessionSecret:'test-session-secret-long-enough',dataEncryptionKey:'test-data-key-long-enough',modelMode:'mock',betaCampaignQuota:10000}});const current=user(context,'safety_user')
...
test('模型失败不核销免费分析',async()=>{
  const gateway={analyze:async()=>{throw new Error('synthetic model failure')}}
  const context=createContext({databaseUrl:':memory:',gateway,config:{nodeEnv:'test',allowDevAuth:false,sessionSecret:'test-session-secret-long-enough',dataEncryptionKey:'test-data-key-long-enough',modelMode:'mock',betaCampaignQuota:10000}});const current=user(context,'failure_user')
...
test('超过单次3狗头硬上限不交付且不核销',async()=>{
  ...
  const context=createContext({databaseUrl:':memory:',gateway,config:{nodeEnv:'test',allowDevAuth:false,sessionSecret:'test-session-secret-long-enough',dataEncryptionKey:'test-data-key-long-enough',modelMode:'mock',betaCampaignQuota:10000,maxWeightedTokens:30000}});const current=user(context,'limit_user')
...
test('内测名额外用户没有免费分析或正式版活动额度',()=>{
  const context=createContext({databaseUrl:':memory:',config:{nodeEnv:'test',allowDevAuth:false,sessionSecret:'test-session-secret-long-enough',dataEncryptionKey:'test-data-key-long-enough',modelMode:'mock',betaCampaignQuota:0}})
  ...
  assert.throws(()=>context.analysis.create(current,payload('这是一份名额外普通用户不应在免费阶段调用模型的合成关系问题，长度足够。')),/没有可用的免费分析/)
})
